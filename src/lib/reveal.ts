 ...
 * frame, then fades back in over `duration` (a CSS time, default
 * var(--dur-long)) --ease-quad-inout after `delay`.
...
export function reveal(
	...
	el.style.transition = `opacity ${options.duration ?? 'var(--dur-long)'} var(--ease-quad-inout) ${options.delay ?? 0}ms`;
	el.style.opacity = '1';
